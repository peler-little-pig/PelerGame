...
class ThingInfoBar(BaseInfoBar):
    def __init__(self, thing):
        ...
        self.attack = Label(self.manager, 480, 10, 500, 30, str(self.thing.egg_type(0,0,0,0,0,0,0).hurt))
        self.energy_icon = pygame.image.load('./Res/Picture/image/energy_coin/energy_coin.png').convert_alpha()
        self.energy = Label(self.manager, 580, 10, 500, 30, str(self.thing.cost))

    def draw(self):
        GameData.surface.blit(self.attack_icon, self)
        GameData.surface.blit(self.energy_icon, pygame.rect.Rect(550, 15, 0, 0))
        self.manager.draw()
        self.manager.update()
